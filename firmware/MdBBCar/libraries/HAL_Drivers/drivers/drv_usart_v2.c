                                       ...
                                       rt_uint32_t           tx_flag)
{
    ...
    if (uart->uart_dma_flag & RT_DEVICE_FLAG_DMA_TX)
    {
        ...
        SCB_CleanDCache_by_Addr((uint32_t *)tx_fifo->buffer, serial->config.tx_bufsz);
#endif
...
static void dma_recv_isr(struct rt_serial_device *serial, rt_uint8_t isr_flag)
{
    ...
    if (recv_len)
    {
        ...
        // SCB_InvalidateDCache_by_Addr((uint32_t *)rx_fifo, sizeof(struct rt_serial_rx_fifo) + serial->config.rx_bufsz);
#endif
