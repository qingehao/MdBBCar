...
/* 预留4K静态AXIRAM给串口DMA用 */
#define AXI_RAM_FOR_UART_DMA_SIZE 4096
#define DCACHE_LINE_SIZE 32

__aligned(DCACHE_LINE_SIZE) static uint8_t axi_ram_dma[AXI_RAM_FOR_UART_DMA_SIZE] __attribute__((section(".axiram_data"))) = {0};
static volatile uint32_t axi_ram_dma_alloc_size = 0;
                                           ...
                                           rt_uint16_t       tx_oflag)
{
    ...
    {
        uint8_t unaligned_size = serial->config.tx_bufsz % DCACHE_LINE_SIZE;
        if (unaligned_size != 0) {
            serial->config.tx_bufsz = serial->config.tx_bufsz + DCACHE_LINE_SIZE - unaligned_size;
        }
        if (axi_ram_dma_alloc_size + serial->config.tx_bufsz > AXI_RAM_FOR_UART_DMA_SIZE)
        {
            LOG_E("(%s) not enough ram for dma buf", dev->parent.name);
            return -RT_ENOMEM;
        }
        tx_fifo = (struct rt_serial_tx_fifo *)rt_malloc(sizeof(struct rt_serial_tx_fifo));
        tx_fifo->buffer = &(axi_ram_dma[axi_ram_dma_alloc_size]);
        axi_ram_dma_alloc_size += serial->config.tx_bufsz;
    }
    else
    {
        tx_fifo = (struct rt_serial_tx_fifo *) rt_malloc
                (sizeof(struct rt_serial_tx_fifo));
        tx_fifo->buffer = rt_malloc(serial->config.tx_bufsz);
    }

    RT_ASSERT(tx_fifo != RT_NULL);
    RT_ASSERT(tx_fifo->buffer != RT_NULL);
                                           ...
                                           rt_uint16_t       rx_oflag)
{
    ...
    {
        uint8_t unaligned_size = serial->config.rx_bufsz % DCACHE_LINE_SIZE;
        if (unaligned_size != 0) {
            serial->config.rx_bufsz = serial->config.rx_bufsz + DCACHE_LINE_SIZE - unaligned_size;
        }
        if (axi_ram_dma_alloc_size + serial->config.rx_bufsz > AXI_RAM_FOR_UART_DMA_SIZE)
        {
            LOG_E("(%s) not enough ram for dma buf", dev->parent.name);
            return -RT_ENOMEM;
        }
        rx_fifo = (struct rt_serial_rx_fifo *)rt_malloc(sizeof(struct rt_serial_rx_fifo));
        rx_fifo->buffer = &(axi_ram_dma[axi_ram_dma_alloc_size]);
        axi_ram_dma_alloc_size += serial->config.rx_bufsz;
    }
    else
    {
        rx_fifo = (struct rt_serial_rx_fifo *) rt_malloc
                (sizeof(struct rt_serial_rx_fifo));
        rx_fifo->buffer = rt_malloc(serial->config.rx_bufsz);
    }
...
static rt_err_t rt_serial_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    if ((oflag & RT_DEVICE_FLAG_DMA_RX) == RT_DEVICE_FLAG_DMA_RX)
        dev->open_flag |= RT_DEVICE_FLAG_DMA_RX;

    if ((oflag & RT_DEVICE_FLAG_DMA_TX) == RT_DEVICE_FLAG_DMA_TX)
